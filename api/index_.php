...
switch (@$_GET['action']) {
    ...
    default:
        ...
        $movieResult = $mysql->query("select DISTINCT c.*,b.*,a.distance, (b.Prob/(b.AVG_DISTANCE*a.distance)) as Points from in_memory_cluster a, cluster_recommendations b, offer_detail c where a.cluster = b.cluster and b.offerid = c.offerid and a.UserID = " . $_SESSION['userid'] . " and b.OfferID not in (select DISTINCT MovieID from demo_rating c WHERE c.UserID = a.UserID) GROUP BY b.OfferID, b.Cluster Order By Points desc LIMIT " . $_SESSION['nmovies']);
        $movieCount = 0;
        ...
        while ($row = $movieResult->fetch_assoc()) {
			...
			$recommendedList[$movieCount]['imddVotes'] = floatval($row["Imdb_votes"]);
			...
			if($row["Poster"])
				$recommendedList[$movieCount]['picture'] = "http://api.klaudnabli.com/reco/cache.php?oid=" . intval($row["OfferID"]);
...
function getSimilarOffers($offerID, $connection, $limit = 20) {
	$similarOffers = array();
	...
	while ($row = $movieResult->fetch_assoc()) {
		...
		if($row["Poster"])
			$similarOffers[$offerCount]['picture'] = "http://api.klaudnabli.com/reco/cache.php?oid=" . intval($row["OfferID"]);
